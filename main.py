...
# ---------------- TP/SL TRACKER ----------------
async def check_tp_sl_status():
    """Check all active signals for TP/SL hits"""
    try:
        async with db_lock:
            # Get all active signals
            async with db_conn.execute(
                "SELECT id, symbol, side, entry_price, sl_price, tp1_price, tp2_price, tp3_price, status FROM signals WHERE status = 'DETECTED'"
            ) as cursor:
        ...
        for signal in active_signals:
            signal_id, symbol, side, entry_price, sl_price, tp1_price, tp2_price, tp3_price, status = signal
            ...
            try:
                ...
                if side == "BUY":
                    ...
                    if current_price <= sl_price:
                        ...
                        pnl_percent = -100.0  # Full loss
                    
                    # Check TP1
                    elif current_price >= tp1_price:
                        closed_reason = "TP1_HIT"
                        pnl_percent = ((tp1_price - entry_price) / entry_price) * 100
                    
                    # Check TP2
                    elif tp2_price and current_price >= tp2_price:
                        closed_reason = "TP2_HIT"
                        pnl_percent = ((tp2_price - entry_price) / entry_price) * 100
                    
                    # Check TP3
                    elif tp3_price and current_price >= tp3_price:
                        closed_reason = "TP3_HIT"
                        pnl_percent = ((tp3_price - entry_price) / entry_price) * 100
                    ...
                    # Check TP1
                        ...
                        closed_reason = "TP1_HIT"
                        pnl_percent = ((entry_price - tp1_price) / entry_price) * 100
                    ...
                    # Check TP3
                        ...
                        closed_reason = "TP3_HIT"
                        pnl_percent = ((entry_price - tp3_price) / entry_price) * 100
                
                # Update database if TP/SL hit
                if closed_reason:
                    async with db_lock:
                        await db_conn.execute(
                            "UPDATE signals SET status = 'CLOSED', closed_at = ?, closed_reason = ?, pnl_percent = ? WHERE id = ?",
                            (datetime.datetime.utcnow().isoformat(), closed_reason, pnl_percent, signal_id)
                        )
                        await db_conn.commit()
...
<b>Symbol:</b> {symbol}
<b>Side:</b> {side}
<b>Status:</b> {closed_reason}
<b>Entry:</b> {entry_price:.8f}
<b>Closed:</b> {current_price:.8f}
<b>PNL:</b> {pnl_percent:.2f}%

<i>Time: {datetime.datetime.utcnow().strftime('%H:%M:%S UTC')}</i>
"""
                    await send_telegram(msg)
                    log.info(f"Signal {signal_id} closed: {closed_reason} ({pnl_percent:.2f}%)")
                
            except Exception as e:
                log.error(f"Error checking TP/SL for {symbol}: {e}")
                continue
    
    except Exception as e:
        log.error(f"Error in TP/SL tracker: {e}")
    ...
    async with db_lock:
        await db_conn.execute("""
            INSERT INTO signals VALUES (
                ...
                :current_price, 'DETECTED', NULL, NULL, NULL
            )
        """, {
            "symbol": setup["symbol"],
...
# ---------------- MAIN SCANNER WITH TP/SL TRACKING ----------------
async def scanner_main(exchange):
    """Main scanning loop with TP/SL tracking"""
    
    await send_telegram("🚀 ROMEOTPT v2 Scanner Started")
    
    while True:
        try:
            # Check TP/SL for existing signals
            await check_tp_sl_status()
...
async def get_active_signals():
    async with db_lock:
        async with db_conn.execute(
            """SELECT id, symbol, side, entry_price, current_price, sl_price, 
                      tp1_price, tp2_price, tp3_price, timestamp 
               FROM signals WHERE status = 'DETECTED'"""
        ) as cursor:
            columns = [description[0] for description in cursor.description]
            rows = await cursor.fetchall()
