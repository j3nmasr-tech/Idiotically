...
class MomentumDivergenceEngine:
    ...
    def analyze_momentum(self, df_1h, df_15m, trend_bias, wave) -> MomentumSignals:
        mom = MomentumSignals()
        if df_1h is None or df_15m is None: return mom
        try:
            ...
            mom.divergence_type = divt; mom.divergence_strength = divs; mom.divergence_points = divp
            if len(macd_15m['macd_line'])>=2:
                pm = macd_15m['macd_line'][-2]; ps = macd_15m['signal_line'][-2]
                cm = macd_15m['macd_line'][-1]; cs = macd_15m['signal_line'][-1]
                mom.macd_line=cm; mom.macd_signal_line=cs; mom.macd_histogram=macd_15m['histogram'][-1]
                if pm<ps and cm>cs:
                    mom.macd_crossed = True; mom.macd_cross_direction = "BULLISH"
                elif pm>ps and cm<cs:
                    mom.macd_crossed = True; mom.macd_cross_direction = "BEARISH"
                if len(macd_15m['histogram'])>=3:
                    h3,h2,h1 = macd_15m['histogram'][-3], macd_15m['histogram'][-2], macd_15m['histogram'][-1]
                    if trend_bias==TrendBias.BULLISH and h3<h2<h1 and h3<0: mom.macd_histogram_reversal = True
                    if trend_bias==TrendBias.BEARISH and h3>h2>h1 and h3>0: mom.macd_histogram_reversal = True
    ...
    def _bull_div(self, df_15m, rsi_15m, df_1h, rsi_1h):
        try:
            ...
            rp = prices[-lookback:]; rr = rsi_15m[-lookback:]
            lows = self._local_lows(rp)
            if len(lows)>=2:
                last=lows[-1]; prev=lows[-2]
            ...
            if len(rsi_1h)>=3 and rsi_1h[-1]>40: return DivergenceType.BULLISH_REGULAR, 0.4, []
        except: pass
    ...
    def _bear_div(self, df_15m, rsi_15m, df_1h, rsi_1h):
        try:
            ...
            if len(rsi_1h)>=3 and rsi_1h[-1]<60: return DivergenceType.BEARISH_REGULAR, 0.4, []
        except: pass
        return DivergenceType.NONE, 0.0, []
...
class VolumeBreakoutTrigger:
    ...
    def detect_breakout(self, df_5m, df_15m, trend_bias, wave, entry_price):
        brk = VolumeBreakout()
        if df_5m is None or len(df_5m)<self.volume_lookback+3: return brk
        if df_15m is None or len(df_15m)<5: return brk
        try:
            rv = df_5m['volume'].values[-(self.volume_lookback+3):]
            latest = rv[-3:]
            avg_vol = np.mean(rv[:self.volume_lookback])
            brk.avg_volume_20 = avg_vol
            if avg_vol<=0: return brk
            for i in range(3):
                cvol = latest[i]
                ratio = cvol/avg_vol
                if ratio < self.min_volume_ratio: continue
                try: candle = df_5m.iloc[-(3-i)]
                except IndexError: continue
                co, cc, ch, cl = candle['open'], candle['close'], candle['high'], candle['low']
                if wave.in_optimal_zone or wave.distance_to_zone_pct<2.0:
                    if trend_bias==TrendBias.BULLISH and (cc>wave.fib_500 or ch>wave.fib_500):
                        brk.triggered=True; brk.breakout_direction="BULLISH"; brk.breakout_price=cc
                        brk.pattern_break=True; brk.volume_ratio=ratio; brk.breakout_candle_volume=cvol
                        brk.volume_score=min(1.0,(ratio-0.8)/3.5)
                        if cl<wave.fib_705*0.998: brk.sweep_then_reclaim=True; brk.volume_score+=0.2; brk.volume_score=min(1.0,brk.volume_score)
                        return brk
                    if trend_bias==TrendBias.BEARISH and (cc<wave.fib_500 or cl<wave.fib_500):
                        brk.triggered=True; brk.breakout_direction="BEARISH"; brk.breakout_price=cc
                        brk.pattern_break=True; brk.volume_ratio=ratio; brk.breakout_candle_volume=cvol
                        brk.volume_score=min(1.0,(ratio-0.8)/3.5)
                        if ch>wave.fib_705*1.002: brk.sweep_then_reclaim=True; brk.volume_score+=0.2; brk.volume_score=min(1.0,brk.volume_score)
                        return brk
                # flag breakout
                if self._is_flag(df_15m, trend_bias==TrendBias.BULLISH):
                    fh = df_15m['high'].iloc[-5:].max(); fl = df_15m['low'].iloc[-5:].min()
                    if trend_bias==TrendBias.BULLISH and cc>fh:
                        brk.triggered=True; brk.breakout_direction="BULLISH"; brk.breakout_price=cc
                        brk.pattern_break=True; brk.volume_ratio=ratio; brk.breakout_candle_volume=cvol
                        brk.volume_score=min(1.0,(ratio-0.8)/2.5); brk.sweep_then_reclaim=(cl<fl*0.998)
                        return brk
                    if trend_bias==TrendBias.BEARISH and cc<fl:
                        brk.triggered=True; brk.breakout_direction="BEARISH"; brk.breakout_price=cc
                        brk.pattern_break=True; brk.volume_ratio=ratio; brk.breakout_candle_volume=cvol
                        brk.volume_score=min(1.0,(ratio-0.8)/2.5); brk.sweep_then_reclaim=(ch>fh*1.002)
                        return brk
            # final fallback
            for i in range(3):
                cvol = latest[i]; ratio = cvol/avg_vol
                if ratio>=self.min_volume_ratio*1.2:
                    try: candle = df_5m.iloc[-(3-i)]
                    except IndexError: continue
                    body = abs(candle['close']-candle['open']); tr = candle['high']-candle['low']
                    if tr>0 and body/tr>0.6:
                        if trend_bias==TrendBias.BULLISH and candle['close']>candle['open']:
                            brk.triggered=True; brk.breakout_direction="BULLISH"; brk.breakout_price=candle['close']
                            brk.volume_ratio=ratio; brk.breakout_candle_volume=cvol; brk.volume_score=0.5
                            return brk
                        if trend_bias==TrendBias.BEARISH and candle['close']<candle['open']:
                            brk.triggered=True; brk.breakout_direction="BEARISH"; brk.breakout_price=candle['close']
                            brk.volume_ratio=ratio; brk.breakout_candle_volume=cvol; brk.volume_score=0.5
                            return brk
        except Exception as e:
...
def identify_liquidity_pools(df, timeframe="1h"):
    ...
    for k in pools:
        if pools[k]:
            ...
            pools[k]=uniq
            if k in ['buy_stops','equal_lows']: pools[k].sort(key=lambda x:x['price'])
            else: pools[k].sort(key=lambda x:x['price'], reverse=True)
    return pools
...
async def calculate_liquidity_tp_sl(exchange, symbol, side, entry_price, entry_type):
    ...
    df_4h = create_dataframe(ohlcv_4h); df_1h = create_dataframe(ohlcv_1h); df_15m = create_dataframe(ohlcv_15m)
    ...
    for p in pools_4h['buy_stops']: p['weight']=3.0; all_pools['buy_stops'].append(p)
    for p in pools_1h['buy_stops']: p['weight']=2.0; all_pools['buy_stops'].append(p)
    for p in pools_15m['buy_stops']: p['weight']=1.0; all_pools['buy_stops'].append(p)
    for pt in ['sell_stops','equal_highs','equal_lows']:
        for p in pools_4h[pt]: p['weight']=3.0; all_pools[pt].append(p)
        for p in pools_1h[pt]: p['weight']=2.0; all_pools[pt].append(p)
        for p in pools_15m[pt]: p['weight']=1.0; all_pools[pt].append(p)
    ...
    cp = entry_price; tp_targets=[]; tp_sources=[]; sl_price=0.0; sl_source={}
    if side=="BUY":
        ...
        if sell_stops_below:
            for tfw in [3.0,2.0,1.0]:
                ...
                if tf_pools: best=min(tf_pools, key=lambda x:x['price']); sl_price=best['price']*0.997; sl_source={'type':'sell_stop_pool','timeframe':best.get('timeframe',''),'reason':best.get('reason',''),'strength':best.get('strength',1),'original_price':best['price']}; break
            if sl_price==0: best=min(sell_stops_below, key=lambda x:x['price']); sl_price=best['price']*0.995
        else:
            eq_lows = [p for p in all_pools['equal_lows'] if p['price']<cp]
            if eq_lows: best=max(eq_lows, key=lambda x:x.get('candle_index',0)); sl_price=best['price']*0.99
            else: return sl_price,[],[],{}
        if sl_price>cp*0.995: sl_price=cp*0.985
        buy_stops_above = [p for p in all_pools['buy_stops'] if p['price']>cp]
        if buy_stops_above:
            tp1=min(buy_stops_above, key=lambda x:x['price']); tp_targets.append(tp1['price'])
            tp_sources.append({'tp_level':1,'type':'buy_stop_pool','timeframe':tp1.get('timeframe',''),'reason':tp1.get('reason',''),'strength':tp1.get('strength',1)})
            above_tp1 = [p for p in all_pools['buy_stops'] if p['price']>tp_targets[0]*1.01]
            if above_tp1: tp2=min(above_tp1, key=lambda x:x['price']); tp_targets.append(tp2['price']); tp_sources.append({'tp_level':2,'type':'buy_stop_pool','timeframe':tp2.get('timeframe',''),'reason':'next_pool','strength':tp2.get('strength',1)})
        else: return sl_price,[],[],{}
    else:
        buy_stops_above = [p for p in all_pools['buy_stops'] if p['price']>cp]
        if buy_stops_above:
            for tfw in [3.0,2.0,1.0]:
                ...
                if tf_pools: best=max(tf_pools, key=lambda x:x['price']); sl_price=best['price']*1.003; sl_source={'type':'buy_stop_pool','timeframe':best.get('timeframe',''),'reason':best.get('reason',''),'strength':best.get('strength',1)}; break
            if sl_price==0: best=max(buy_stops_above, key=lambda x:x['price']); sl_price=best['price']*1.005
        else:
            eq_highs = [p for p in all_pools['equal_highs'] if p['price']>cp]
            if eq_highs: best=max(eq_highs, key=lambda x:x.get('candle_index',0)); sl_price=best['price']*1.01
            else: return sl_price,[],[],{}
        if sl_price<cp*1.005: sl_price=cp*1.015
        sell_stops_below = [p for p in all_pools['sell_stops'] if p['price']<cp]
        if sell_stops_below:
            tp1=max(sell_stops_below, key=lambda x:x['price']); tp_targets.append(tp1['price'])
            tp_sources.append({'tp_level':1,'type':'sell_stop_pool','timeframe':tp1.get('timeframe',''),'reason':tp1.get('reason',''),'strength':tp1.get('strength',1)})
            below_tp1 = [p for p in all_pools['sell_stops'] if p['price']<tp_targets[0]*0.99]
            if below_tp1: tp2=max(below_tp1, key=lambda x:x['price']); tp_targets.append(tp2['price']); tp_sources.append({'tp_level':2,'type':'sell_stop_pool','timeframe':tp2.get('timeframe',''),'reason':'next_pool','strength':tp2.get('strength',1)})
        else: return sl_price,[],[],{}
    risk = abs(cp-sl_price); reward = abs(tp_targets[0]-cp) if tp_targets else 0; rr = reward/risk if risk>0 else 0
    liq_analysis = {'side':side,'entry_type':entry_type,'identified_pools':{k:len(v) for k,v in all_pools.items()},'sl_source':sl_source,'tp_sources':tp_sources,'rr_ratio':rr,'risk_pct':risk/cp*100 if cp>0 else 0,'reward_pct':reward/cp*100 if cp>0 and tp_targets else 0}
...
class InstitutionalDataFetcher:
    ...
    async def get_institutional_data(self, exchange, symbol: str) -> InstitutionalData:
        ckey = f"{symbol}_inst"; now=time.time()
        ...
        try:
            ...
            if not isinstance(fd, Exception) and fd: data.funding_rate = fd.get('fundingRate',0)*100; data.funding_timestamp=datetime.datetime.utcnow()
            if not isinstance(oi, Exception) and oi: data.open_interest = oi.get('openInterest',0); data.oi_timestamp=datetime.datetime.utcnow()
    ...
    async def _fetch_funding(self, ex, symbol): 
        try: return await rate_limiter.execute_with_backoff(ex.fetch_funding_rate, symbol, endpoint_type="funding")
        except: return {}
    async def _fetch_oi(self, ex, symbol): 
        try: return await rate_limiter.execute_with_backoff(ex.fetch_open_interest, symbol, endpoint_type="oi")
        except: return {}
    async def get_orderbook_bias(self, exchange, symbol: str) -> float:
        try:
            ...
            if ask_vol>0: return bid_vol/ask_vol
        except: pass
        return 1.0
...
class DirectionEngine:
    ...
    async def analyze_direction(self, exchange, symbol, proposed_side, current_price) -> DirectionMetrics:
        ...
        try:
            ...
            ts, tc = self._quick_trapped(inst, proposed_side, current_price); metrics.trapped_side=ts; metrics.trapped_confidence=tc
            bs, fe = self._quick_bleeding(inst); metrics.bleeding_side=bs; metrics.funding_extreme=fe
            ds = 0.0
            if proposed_side=="BUY":
                if ts==TrappedSide.SHORT: ds+=0.3
                if bs=="LONG": ds+=0.2
            else:
                if ts==TrappedSide.LONG: ds+=0.3
                if bs=="SHORT": ds+=0.2
            absd = abs(ds)
            if absd>0.4: metrics.confidence_tier = DirectionTier.HIGH
            elif absd>0.2: metrics.confidence_tier = DirectionTier.MEDIUM
        ...
        except Exception as e: log.debug(f"dir engine error: {e}")
        return metrics
    def _quick_trapped(self, inst, side, price):
        ...
        if inst.oi_change_24h < -OI_ACCUMULATION_THRESHOLD*100 and inst.funding_rate < -FUNDING_EXTREME_THRESHOLD: return TrappedSide.SHORT, 0.6
    ...
    def _quick_bleeding(self, inst):
        ...
        if inst.funding_rate < -FUNDING_EXTREME_THRESHOLD: return "SHORT", abs(inst.funding_rate)
        return "", 0.0
...
class FastMomentumScalper:
    def __init__(self):
        self.min_vol_ratio = 1.5; self.min_body_pct = 0.3; self.min_rr = 2.0; self.ema_fast=9; self.ema_slow=21
    async def scan(self, exchange, symbol, current_price, trend_bias_hint=TrendBias.NEUTRAL):
        try:
            ...
            if df_3m is None or len(df_3m)<30: return None
            closes = df_3m['close'].values; highs=df_3m['high'].values; lows=df_3m['low'].values; vols=df_3m['volume'].values; opens=df_3m['open'].values
            avg_vol = np.mean(vols[-22:-2]) if len(vols)>=22 else np.mean(vols[:-2])
            last_vol = vols[-1]
            if avg_vol<=0 or last_vol<avg_vol*self.min_vol_ratio: return None
            ema9 = pd.Series(closes).ewm(span=self.ema_fast).mean().iloc[-1]
            ema21 = pd.Series(closes).ewm(span=self.ema_slow).mean().iloc[-1]
            rsi = self._rsi(closes,14)
            body = abs(closes[-1]-opens[-1]); wick_high = highs[-1]-max(opens[-1],closes[-1]); wick_low = min(opens[-1],closes[-1])-lows[-1]
            tr = highs[-1]-lows[-1]; if tr==0: return None
            body_ratio = body/tr
            side=None; sl_price=0.0; tp_price=0.0
            if ema9>ema21 and rsi>50 and closes[-1]>opens[-1] and body_ratio>self.min_body_pct and wick_high<body*0.5:
                entry = highs[-1]*1.001; sl = lows[-1]*0.997
                tp = current_price + (current_price-sl)*2.0
                if tp/current_price-1<0.02: tp=current_price*1.025
                if (tp-current_price)/(current_price-sl)<self.min_rr: return None
                side="BUY"; sl_price=sl; tp_price=tp
            elif ema9<ema21 and rsi<50 and closes[-1]<opens[-1] and body_ratio>self.min_body_pct and wick_low<body*0.5:
                entry = lows[-1]*0.999; sl = highs[-1]*1.003
                tp = current_price - (sl-current_price)*2.0
                if 1-tp/current_price<0.02: tp=current_price*0.975
                if (current_price-tp)/(sl-current_price)<self.min_rr: return None
                side="SELL"; sl_price=sl; tp_price=tp
            if side is None: return None
            return {"symbol":symbol,"timestamp":datetime.datetime.utcnow().isoformat(),"side":side,"current_price":current_price,
                    "entry_price":current_price,"entry_type":"MOMENTUM_SCALP","sl_price":sl_price,"tp_targets":[tp_price],
                    "risk":abs(current_price-sl_price),"reward":abs(tp_price-current_price),
                    "rr_ratio":abs(tp_price-current_price)/abs(current_price-sl_price) if abs(current_price-sl_price)>0 else 0,
                    "trend_bias":trend_bias_hint.value,"quality":{"total_score":2.2,"tier":"B"},"forced_move_probability":"MODERATE","method":"MOMENTUM_SCALP"}
        except Exception as e:
            log.debug(f"fast scalp error {symbol}: {e}"); return None
    def _rsi(self, prices, period=14):
        delta = np.diff(prices); gain = np.where(delta>0,delta,0); loss = np.where(delta<0,-delta,0)
...
class SignalTracker:
    def __init__(self):
        self.active_signals={}; self.outcome_stats={'total_signals':0,'tp1_hits':0,'tp2_hits':0,'tp3_hits':0,'sl_hits':0,'expired':0,'active':0,'win_rate':0.0,'avg_pnl_pct':0.0}
    def get_signal_key(self, setup): return (setup['symbol'], setup['side'], math.floor(setup.get('quality',{}).get('total_score',0)*2)/2)
    ...
    def update_signal(self, setup, alerted=False):
        ...
        if key not in self.active_signals:
            self.active_signals[key] = {'setup':setup,'first_seen':now,'last_alerted':now if alerted else None,'last_checked':now,'alert_count':1 if alerted else 0,'status':'active','outcome':'active','highest_price':setup.get('current_price',0),'lowest_price':setup.get('current_price',0),'price_at_alert':setup.get('current_price',0) if alerted else None}
    ...
    def check_signal_outcome(self, setup, current_price):
        ...
        if sig.get('status')!='active': return None
        side=setup.get('side',''); entry=setup.get('entry_price',0); tps=setup.get('tp_targets',[]); sl=setup.get('sl_price',0)
        ...
        for i,tp in enumerate(tps):
            ...
            if (side=="BUY" and current_price>=tp) or (side=="SELL" and current_price<=tp):
                ...
                outcome={'type':f'TP{i+1}_HIT','price':current_price,'pnl_pct':pnl}; break
        ...
        if outcome:
            sig['status']='closed'; sig['outcome']=outcome['type'].lower(); self.outcome_stats['active']-=1
            if 'TP1_HIT' in outcome['type']: self.outcome_stats['tp1_hits']+=1
            elif 'TP2_HIT' in outcome['type']: self.outcome_stats['tp2_hits']+=1
            elif 'TP3_HIT' in outcome['type']: self.outcome_stats['tp3_hits']+=1
            ...
            if total>0: self.outcome_stats['win_rate']=wins/total*100
        return outcome
    def remove_signal_by_key(self, key, reason=""):
        if key in self.active_signals: del self.active_signals[key]; self.outcome_stats['active']-=1; self.outcome_stats['expired']+=1
    def cleanup_old_signals(self):
        now=datetime.datetime.utcnow(); expired=[]
        for k,v in self.active_signals.items():
            if v.get('status')=='active' and (now-v['first_seen']).total_seconds()/60 > SIGNAL_VALIDITY_HOURS*60:
                expired.append(k)
        for k in expired: self.remove_signal_by_key(k)
    def get_stats(self):
...
async def send_telegram(msg: str, parse_mode="HTML"):
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        except Exception as e: log.warning(f"Telegram fail: {e}")

async def send_v6_alert(setup: Dict):
    try:
        symbol=setup['symbol']; side=setup['side']; quality=setup.get('quality',{})
        ...
        if qs>=4.5 and wave_conf>=0.7 and mom_score>=0.7 and vol_trig: tier="S+"; emoji="🔮"
        elif qs>=3.5 and mom_score>=0.6 and vol_trig: tier="A+"; emoji="🔥"
        elif qs>=2.5: tier="A"; emoji="✅"
        elif qs>=2.0: tier="B"; emoji="⚠️"
        else: tier="C"; emoji="📊"
        wave_lines=[]
        if isinstance(wave,WaveStructure) and wave.pattern!=WavePattern.NONE:
            ...
            else: wave_lines.append(f"📍 Distance to zone: {wave.distance_to_zone_pct:.1f}%")
        momentum_lines=[]
        if isinstance(momentum,MomentumSignals):
            div_e="🐂" if momentum.divergence_type==DivergenceType.BULLISH_REGULAR else "🐻" if momentum.divergence_type==DivergenceType.BEARISH_REGULAR else "⚪"
            momentum_lines.append(f"{div_e} Divergence: {momentum.divergence_type.value} ({momentum.divergence_strength:.0%})")
            macd_e="✅" if momentum.macd_crossed else "🔄" if momentum.macd_histogram_reversal else "⏳"
            ...
            if momentum.momentum_aligned: momentum_lines.append("🎯 MOMENTUM ALIGNED ✅")
        volume_lines=[]
        if isinstance(volume,VolumeBreakout):
            if volume.triggered:
                volume_lines.append(f"🚀 BREAKOUT: Volume {volume.volume_ratio:.1f}x avg")
                if volume.sweep_then_reclaim: volume_lines.append("🧹 SWEEP + RECLAIM detected!")
            else: volume_lines.append("⏳ Waiting for volume confirmation...")
        tp_lines=[]
        for i,tp in enumerate(tp_targets):
            if entry_price>0: tp_lines.append(f"TP{i+1}: {tp:.8f} ({abs(tp-entry_price)/entry_price*100:.1f}%)")
        dir_ctx=""
        if isinstance(direction,DirectionMetrics) and direction.trapped_side!=TrappedSide.NONE: dir_ctx+=f" | Trapped: {direction.trapped_side.value}"
...
<i>Wave+Momentum+Volume Method | {datetime.datetime.utcnow().strftime('%H:%M:%S UTC')}</i>"""
    ...
    except Exception as e: log.error(f"alert error: {e}")
...
async def store_signal(setup: Dict):
    async with db_lock:
        tp = setup.get('tp_targets',[]); q = setup.get('quality',{})
        wave = setup.get('wave_structure',{}); mom = setup.get('momentum_signals',{}); vol = setup.get('volume_breakout',{})
        direc = setup.get('direction_metrics',{})
        key = signal_tracker.get_signal_key(setup); _,_,bucket = key
        if isinstance(wave,WaveStructure): wp=wave.pattern.value; wc=wave.pattern_confidence; fr=wave.current_retracement; iz=wave.in_optimal_zone
        else: wp=wave.get('pattern','NONE'); wc=wave.get('pattern_confidence',0); fr=wave.get('current_retracement',0); iz=wave.get('in_optimal_zone',False)
        if isinstance(mom,MomentumSignals): dt=mom.divergence_type.value; ds=mom.divergence_strength; ms=mom.momentum_score; mc=mom.macd_crossed; ma=mom.momentum_aligned
        else: dt=mom.get('divergence_type','NONE'); ds=mom.get('divergence_strength',0); ms=mom.get('momentum_score',0); mc=mom.get('macd_crossed',False); ma=mom.get('momentum_aligned',False)
        if isinstance(vol,VolumeBreakout): vt=vol.triggered; vr=vol.volume_ratio; sw=vol.sweep_then_reclaim
        else: vt=vol.get('triggered',False); vr=vol.get('volume_ratio',0); sw=vol.get('sweep_then_reclaim',False)
        if isinstance(direc,DirectionMetrics): dtier=direc.confidence_tier.value; dscore=direc.direction_score; trap=direc.trapped_side.value
        else: dtier=direc.get('confidence_tier','LOW'); dscore=direc.get('direction_score',0); trap=direc.get('trapped_side','NONE')
...
async def scan_symbol_v6(exchange, symbol: str) -> Optional[Dict]:
    try:
        ...
        if df_d is None or df_4h is None or df_1h is None: return None
        ticker = await safe_fetch_ticker(exchange, symbol)
        if not ticker: return None
        cp = ticker.get('last',0); if cp<=0: return None
        ...
        if wave.pattern == WavePattern.NONE or wave.pattern_confidence<0.3: return None
        ...
        if side=="BUY" and ob_bias>1.5: vol_breakout.volume_score = min(1.0, vol_breakout.volume_score+0.1)
        elif side=="SELL" and ob_bias<0.67: vol_breakout.volume_score = min(1.0, vol_breakout.volume_score+0.1)
        ...
        if wave.in_optimal_zone: qs += 0.5
        if dir_metrics.confidence_tier==DirectionTier.HIGH: qs += 0.5
        elif dir_metrics.confidence_tier==DirectionTier.MEDIUM: qs += 0.3
        if qs<MIN_QUALITY_SCORE: return None
        tier = "S+" if qs>=4.5 else "A+" if qs>=4.0 else "A" if qs>=3.0 else "B" if qs>=2.5 else "C"
        return {"symbol":symbol,"timestamp":datetime.datetime.utcnow().isoformat(),"side":side,"current_price":cp,
        ...
        log.error(f"scan error {symbol}: {e}"); return None
...
async def main_scan_with_fallback(exchange, symbol):
    ...
    hint = TrendBias.BULLISH if (df_15m is not None and len(df_15m)>20 and df_15m['close'].iloc[-1]>df_15m['close'].iloc[-20:].mean()) else TrendBias.BEARISH if df_15m is not None else TrendBias.NEUTRAL
    scalp = await fast_scalper.scan(exchange, symbol, cp, hint)
    if scalp:
        try: scalp['direction_metrics'] = await direction_engine.analyze_direction(exchange, symbol, scalp['side'], cp)
        except: scalp['direction_metrics'] = DirectionMetrics()
        return scalp
    return None

class OutcomeMonitor:
    def __init__(self, exchange, interval=OUTCOME_CHECK_INTERVAL):
        self.exchange = exchange; self.interval = interval
    async def monitor_loop(self):
        ...
        while True:
            try:
                ...
                if not rows: await asyncio.sleep(self.interval); continue
                syms = set(r[0] for r in rows); tickers={}
                for s in syms:
                    t = await safe_fetch_ticker(self.exchange, s)
                    if t: tickers[s]=t.get('last',0)
                for (sym,side,entry,sl,tp1,tp2,tp3,sig_id) in rows:
                    price = tickers.get(sym)
                    if not price or price<=0: continue
                    outcome=None; tp_lv=0
                    if (side=="BUY" and price<=sl) or (side=="SELL" and price>=sl): outcome="SL_HIT"
                    elif tp1 and ((side=="BUY" and price>=tp1) or (side=="SELL" and price<=tp1)): outcome="TP1_HIT"; tp_lv=1
                    elif tp2 and ((side=="BUY" and price>=tp2) or (side=="SELL" and price<=tp2)): outcome="TP2_HIT"; tp_lv=2
                    elif tp3 and ((side=="BUY" and price>=tp3) or (side=="SELL" and price<=tp3)): outcome="TP3_HIT"; tp_lv=3
                    if outcome:
                        ...
                        if tp_lv: msg+=f"\nTP{tp_lv}: <code>{[tp1,tp2,tp3][tp_lv-1]:.8f}</code>"
                        await send_telegram(msg)
                        async with db_lock:
                            await db_conn.execute("UPDATE signals_v6_1 SET status='closed',outcome=?,closed_at=?,closed_price=?,pnl_pct=? WHERE id=?",(outcome,datetime.datetime.utcnow().isoformat(),price,pnl,sig_id))
                            await db_conn.commit()
            except Exception as e: log.error(f"outcome monitor error: {e}")
...
async def v6_scanner_main(exchange):
    ...
    while True:
        ...
        try:
            ...
            for sym, data in tickers.items():
                if sym.endswith("USDT:USDT") and not sym.startswith("USDT"):
                    ...
                    if vol is None or vol == 0:
                        ...
                        if base and last: vol = base * last
            ...
            for sym in symbols:
                ...
                if len(tasks) >= 5:
                    ...
                    for r in results:
                        ...
                        if r:
                            if await send_deduped_v6_alert(r): alerts_this += 1
            ...
            if scan_cycle % 5 == 0:
                os = stats.get('outcome_stats',{})
                wins = os.get('tp1_hits',0)+os.get('tp2_hits',0)+os.get('tp3_hits',0)
                losses = os.get('sl_hits',0)
                if wins+losses>0: log.info(f"📈 WR={os.get('win_rate',0):.1f}% Active={os.get('active',0)}")
...
async def health():
    stats = signal_tracker.get_stats()
    return {"status":"healthy","version":"6.1 MEXC Futures","active_signals":stats['active_signals'],"outcome_stats":stats['outcome_stats']}
...
async def get_active():
    ...
    for k,v in signal_tracker.active_signals.items():
        if v.get('status')=='active':
            sym,side,bucket = k
            st = v.get('setup',{})
            act.append({"symbol":sym,"side":side,"quality_score":st.get('quality',{}).get('total_score',0),"entry":st.get('entry_price',0),"sl":st.get('sl_price',0),"tp1":st.get('tp_targets',[0])[0] if len(st.get('tp_targets',[]))>0 else 0,"rr":st.get('rr_ratio',0)})
