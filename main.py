...
# Outcome monitoring interval (seconds)
...
# ============ SIGNAL TRACKER (PRESERVED + extended) ============
class SignalTracker:
    def __init__(self):
        self.active_signals = {}
        self.outcome_stats = {
...
# ============ NEW: OUTCOME ALERT ============
async def send_outcome_alert(symbol: str, side: str, outcome: Dict, setup: Dict):
    """
    Sends a Telegram notification when a signal reaches TP or SL.
    Includes outcome type, time to hit, and PnL.
    """
    try:
        entry_price = setup.get('entry_price', 0)
        tp_targets = setup.get('tp_targets', [])
        ...
        forced = setup.get('forced_move_probability', 'LOW')
        
        outcome_type = outcome.get('type', 'UNKNOWN')
        pnl = outcome.get('pnl_pct', 0.0)
        bars_held = outcome.get('bars_held', 0)  # in minutes
        ...
        msg = f"""{emoji} <b>OUTCOME: {outcome_type}</b> on <b>{symbol}</b>
        
<b>Side:</b> {side}
<b>Entry Price:</b> <code>{entry_price:.8f}</code>
<b>Exit Price:</b> <code>{current_price:.8f}</code>
<b>PnL:</b> {result_text}

⏱️ <b>Time to Hit:</b> {bars_held} minutes
📊 <b>Quality Tier:</b> {tier} | Forced Move: {forced}
...
# ============ NEW: OUTCOME MONITOR (background task) ============
async def outcome_monitor(exchange):
    """
    Periodically checks active signals for TP/SL hits and sends outcome alerts.
    """
    log.info("🔍 Outcome monitor started")
    while True:
        try:
            active_signals = {k: v for k, v in signal_tracker.active_signals.items()
                              if v.get('status') == 'active'}
            if not active_signals:
                await asyncio.sleep(OUTCOME_CHECK_INTERVAL)
                continue
            
            # Fetch current prices for all active symbols (we can fetch them one by one)
            for key, signal_data in active_signals.items():
                ...
                try:
                    ...
                    if outcome:
                        log.info(f"Outcome detected for {symbol}: {outcome['type']}")
                        await send_outcome_alert(symbol, setup.get('side', ''), outcome, setup)
                        # Optionally update the database as well
                        # (storing outcome in DB not implemented here, but could be added)
                except Exception as e:
...
async def main():
    ...
    try:
        ...
        log.info("Outcome monitoring enabled")
        
        # Start the outcome monitor as a background task
        asyncio.create_task(outcome_monitor(exchange))
