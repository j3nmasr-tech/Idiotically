...
# ---------------- SIMPLIFIED ROMEOPT TP DECISION (FIXED VERSION) ----------------
def romeopt_tp_sl(entry, side, atr_val, ob_zone, df):
    """
    SIMPLIFIED RomeOPT TP logic - CONFIRMATION ONLY, NOT FILTER
    """
    # Step 1: Find nearest obvious swing liquidity (SIMPLE)
    tp = None
    lookback = 20  # Look for obvious swings in recent price action
    
    if side == "SELL":
        # For SELL: Look for obvious swing low
        recent_lows = df['low'].iloc[-lookback:].dropna()
        if len(recent_lows) > 0:
            # Take the most obvious low (minimum)
            tp = recent_lows.min()
            tp_type = "Nearest Swing Low"
    else:  # BUY
        # For BUY: Look for obvious swing high
        recent_highs = df['high'].iloc[-lookback:].dropna()
        if len(recent_highs) > 0:
            # Take the most obvious high (maximum)
            tp = recent_highs.max()
            tp_type = "Nearest Swing High"
    
    # Step 2: If no obvious swing, use range extreme as fallback
    if tp is None:
        range_lookback = 50
        if side == "SELL":
            tp = df['low'].iloc[-range_lookback:].min()
            tp_type = "Range Low"
        else:
            tp = df['high'].iloc[-range_lookback:].max()
            tp_type = "Range High"
    
    # REJECT only if truly no target (should be rare)
    if tp is None:
        log.debug(f"❌ No reasonable target found for {side}")
        return None
    
    # Step 3: Calculate SL (keep OB-based SL)
    if side == "BUY":
        ...
        # Ensure TP is above entry (RomeOPT: liquidity is opposite side)
        if tp <= entry:
            # Adjust to at least 0.5R above entry
            tp = entry + risk * 0.5
            tp_type += " (Adjusted)"
    ...
    log.info(f"✅ {side} {entry:.6f} | TP: {tp_type}")
    log.info(f"   SL: {sl:.6f} | TP: {tp:.6f}")
...
async def generate_signal_romeopt(exchange, df: pd.DataFrame, symbol: str, tf: str):
    ...
    # ---------------- SIMPLIFIED ROMEOPT TP CALCULATION ----------------
    atr_val = float(atr(df, 14).iloc[-1])
    result = romeopt_tp_sl(entry, side_str, atr_val, ob_zone, df)
    
    # REJECT if no valid TP found (rare with simplified logic)
    if result is None:
        # FALLBACK: Use reasonable 2:1 R:R if no obvious target
        reasons.append("⚠️ No optimal liquidity found, using fallback TP")
        ...
        if side_str == "BUY":
            ...
            if risk < min_risk:
                risk = min_risk
                sl = entry - risk
            tp = entry + risk * 2  # 2:1 R:R fallback
            tp_type = "Fallback 2:1 R:R"
        else:  # SELL
            sl = ob_zone["high"] + (atr_val * 0.3)
            recent_high = df['high'].iloc[-10:].max()
            sl = max(sl, recent_high + (atr_val * 0.3))
...
async def main():
    ...
    await tg("🔧 FIXED: Simplified TP logic (confirmation, not filter)")
