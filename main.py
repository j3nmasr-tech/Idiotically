...
"""
LIVE ROMEOPT 6-STEP SCANNER (Enhanced + Elite Features) - FIXED 3% TP/SL
- Fully live early signals
- RomeOPT 6-step logic with NUMERICAL BREAKDOWN
- FIXED 3% TP/SL: Always use 3% for both stop loss and take profit
- Telegram alerts
...
- STRUCTURE-REQUIRED: REMOVED - Now uses fixed percentage
...
- 🎯 ENHANCED BREAKDOWN FORMAT: Updated for fixed 3% TP/SL
"""
...
FIXED_TP_PERCENT = 0.03  # 3% take profit
FIXED_SL_PERCENT = 0.03  # 3% stop loss
...
# ---------------- SIMPLE FIXED 3% TP/SL SYSTEM ----------------
def calculate_fixed_tp_sl(entry: float, side: str):
    """
    Simple fixed 3% TP/SL calculation
    Returns: sl, tp1, risk_reward
    Risk/Reward is always 1:1 with 3% TP and 3% SL
    """
    if side == "BUY":
        tp1 = entry * (1 + FIXED_TP_PERCENT)
        sl = entry * (1 - FIXED_SL_PERCENT)
    else:  # SELL
        tp1 = entry * (1 - FIXED_TP_PERCENT)
        sl = entry * (1 + FIXED_SL_PERCENT)
    
    # Risk/Reward is 1:1 with fixed percentages
    risk_reward = 1.0
    
    return sl, tp1, risk_reward

def update_tp_sl_live(sig: dict, df: pd.DataFrame):
    """
    Calculate FIXED 3% TP/SL - No structure requirement
    """
    entry = sig["entry"]
    side = sig["side"]
    
    # Get fixed TP/SL
    sl, tp1, rr_tp1 = calculate_fixed_tp_sl(entry, side)
    
    # Store TP/SL data in signal
    sig.update({
        "sl": sl,
        "tp1": tp1,
        "tp2": None,  # No TP2 with fixed system
        "tp3": None,  # No TP3 with fixed system
        "atr_value": float(atr(df, 14).iloc[-1]) if df is not None and len(df) > 14 else 0.0,
        "risk_reward": rr_tp1,
        "risk": abs(entry - sl)
    })
...
async def generate_signal_romeopt(exchange, df: pd.DataFrame, symbol: str, tf: str):
    """Generate RomeOPT 6-step signal with FIXED 3% TP/SL"""
    ...
    if "risk" in sig:
        breakdown["risk_management"] = {
            ...
            "tp_system": "FIXED_3%",
            "tp_percent": FIXED_TP_PERCENT,
            "sl_percent": FIXED_SL_PERCENT
        }
...
async def send_enhanced_breakdown(sig):
    """
    Format and send the enhanced breakdown for FIXED 3% TP/SL system
    """
    ...
    breakdown_lines = [
        ...
        f"TP/SL System: 🔥 FIXED 3% (1:1 Risk/Reward)",
        f""
    ]
    ...
    # 🎯 FIXED 3% TARGETS
    breakdown_lines.append(f"🎯 FIXED 3% TARGETS (1:1 Risk/Reward):")
    ...
    if "sl" in sig and sig["sl"]:
        ...
        if sig['side'] == "BUY":
            tp_percent = ((tp1 - entry) / entry * 100) if entry > 0 else 0
            sl_percent = ((entry - sl) / entry * 100) if entry > 0 else 0
        else:  # SELL
            tp_percent = ((entry - tp1) / entry * 100) if entry > 0 else 0
            sl_percent = ((sl - entry) / entry * 100) if entry > 0 else 0
        
        breakdown_lines.extend([
            f"  SL: {format_number(sig.get('sl', 0))} ({sl_percent:.2f}%)",
            f"  TP1: {format_number(sig.get('tp1', 0))} ({tp_percent:.2f}%)",
            f"  Risk/Reward: 1:1",
...
async def monitor_signals():
    """
    Monitor open positions for TP/SL hits - OPTIMIZED FOR FIXED 3%
    - TP/SL levels are FIXED 3% after entry
    - Only TP1 and SL are monitored (TP2/TP3 are None in fixed system)
    - When TP1 hits, position closes automatically (1:1 R/R achieved)
    """
    while True:
        try:
            async with db_lock:
                # OPTIMIZED: Only select necessary columns for fixed 3% system
                async with db_conn.execute("SELECT id, symbol, side, entry, sl, tp1, tp1_hit, status FROM signals WHERE status='OPEN'") as cursor:
                    async for row in cursor:
                        sig_id, symbol, side, entry, sl, tp1, tp1_hit, status = row
                        ...
                        # Check for TP/SL hits - FIXED 3% SYSTEM (only TP1 and SL matter)
                        hits = []
                        sl_hit = False
                        new_tp1_hit = tp1_hit
                        new_status = status
                        ...
                        if side == "BUY":
                            if not tp1_hit and last_price >= tp1:
                                hits.append("TP1")
                                new_tp1_hit = 1
                                new_status = "CLOSED"  # Close on TP1 hit in fixed system
                            elif last_price <= sl:
                                hits.append("SL")
                                new_status = "CLOSED"
                                sl_hit = True
                        ...
                        if hits:
                            # Calculate percentages for informative message
                            if side == "BUY":
                                tp_percent = ((tp1 - entry) / entry * 100) if entry > 0 else 0
                                sl_percent = ((entry - sl) / entry * 100) if entry > 0 else 0
                                current_percent = ((last_price - entry) / entry * 100) if entry > 0 else 0
                            else:  # SELL
                                tp_percent = ((entry - tp1) / entry * 100) if entry > 0 else 0
                                sl_percent = ((sl - entry) / entry * 100) if entry > 0 else 0
                                current_percent = ((entry - last_price) / entry * 100) if entry > 0 else 0
                            
                            # Build informative message
                            msg_lines = [
                                f"🔥 {symbol} {side} - Fixed 3% System",
                                f"══════════════════════════════",
                                f"Entry: {format_number(entry)}",
                                f"Current: {format_number(last_price)} ({current_percent:+.2f}%)",
                                f"══════════════════════════════",
                                f"🎯 HIT: {', '.join(hits)}",
                                f"SL: {format_number(sl)} ({sl_percent:.2f}%)",
                                f"TP1: {format_number(tp1)} ({tp_percent:.2f}%)",
                                f"══════════════════════════════"
                            ]
                            
                            if "TP1" in hits:
                                msg_lines.append(f"💰 PROFIT: {tp_percent:.2f}% (1:1 Risk/Reward)")
                            elif "SL" in hits:
                                msg_lines.append(f"📉 LOSS: {sl_percent:.2f}% (1:1 Risk/Reward)")
                            
                            await tg("\n".join(msg_lines))
                            log.info(f"{symbol} {side}: {', '.join(hits)} at {last_price}")
                        
                        if sl_hit:
                            record_sl_hit(symbol)
                        
                        # Update database only if something changed
                        if new_tp1_hit != tp1_hit or new_status != status:
                            await db_conn.execute(
                                "UPDATE signals SET tp1_hit=?, status=? WHERE id=?",
                                (new_tp1_hit, new_status, sig_id)
                            )
        ...
        try:
            ...
            for symbol, _ in top:
                ...
                for tf in TIMEFRAMES:
                    ...
                    if sig:
                        # Send enhanced breakdown with fixed 3% system
            ...
            log.info(f"📊 Scan complete: {signals_found} RomeOPT signals found (Fixed 3% TP/SL)")
...
async def main():
    ...
    await tg("🏆 ROMEOPT 6-Step Scanner Started - FIXED 3% TP/SL")
    await tg("🎯 TP/SL SYSTEM: ALWAYS 3% for both stop loss and take profit")
    await tg("📈 Risk/Reward: 1:1 (3% profit vs 3% loss)")
    await tg("📊 ENHANCED BREAKDOWN: Shows fixed percentage targets")
