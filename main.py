...
Based on: Market Structure + Order Flow + Short-Term Momentum
"""
...
TOP_N_VOLUME = int(os.getenv("TOP_N_VOLUME", 80))     # Focus on quality pairs
...
MIN_CONFLUENCE_SCORE = 2.0        # Min confluence score (out of 10)
...
class ConfluenceScanner:
    ...
    def analyze_momentum(self, df_15m: pd.DataFrame, df_5m: pd.DataFrame) -> MomentumSignal:
        ...
        try:
            ...
            # 1. RSI analysis
            rsi_divergence, rsi_value, rsi_zone = self._analyze_rsi(df_15m)
            
            # 2. MACD analysis
            macd_signal, macd_histogram = self._analyze_macd(df_15m)
    ...
    def _analyze_rsi(self, df: pd.DataFrame) -> Tuple[str, float, str]:
        ...
        try:
            ...
            if len(rsi_values) < 5 or np.isnan(rsi_values[-1]):
                return "NONE", 50.0, "NEUTRAL"
    ...
    def _analyze_macd(self, df: pd.DataFrame) -> Tuple[str, float]:
        ...
        try:
            ...
            if len(macd) < 2 or np.isnan(macd[-1]):
                return "NONE", 0.0
            
            current_hist = float(hist[-1])
            prev_hist = float(hist[-2]) if len(hist) > 1 else 0.0
            
            # Check for crossover
            current_macd = float(macd[-1])
            current_signal = float(signal[-1])
            prev_macd = float(macd[-2]) if len(macd) > 1 else 0.0
            prev_signal = float(signal[-2]) if len(signal) > 1 else 0.0
